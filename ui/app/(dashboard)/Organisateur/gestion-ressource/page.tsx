...
const iconMap: Record<string, any> = { Mobilier: Package, Technique: Wifi, Logistique: Truck, Structure: CheckCircle };
        ...
        <div className={styles.page}>
            ...
            <div className={styles.statGrid}>
                <div className={styles.statCard}>
                    <div className={styles.statIconBox} style={{ background: 'rgba(36,180,126,0.1)', color: 'var(--color-success)' }}><CheckCircle size={26} /></div>
                    <div><p className={styles.statLabel}>Disponibles</p><p className={styles.statValue}>{available}</p></div>
                </div>
                <div className={styles.statCard}>
                    <div className={styles.statIconBox} style={{ background: 'rgba(255,176,32,0.1)', color: 'var(--color-warning)' }}><AlertTriangle size={26} /></div>
                    <div><p className={styles.statLabel}>Partiels</p><p className={styles.statValue}>{partial}</p></div>
                </div>
                <div className={styles.statCard}>
                    <div className={styles.statIconBox} style={{ background: 'rgba(239,83,80,0.1)', color: 'var(--color-danger)' }}><Package size={26} /></div>
                    <div><p className={styles.statLabel}>Occupés</p><p className={styles.statValue}>{busy}</p></div>
                </div>
                <div className={styles.statCard}>
                    <div className={styles.statIconBox} style={{ background: 'rgba(255,147,79,0.1)', color: 'var(--color-primary)' }}><Package size={26} /></div>
                    <div><p className={styles.statLabel}>Total ressources</p><p className={styles.statValue}>{resources.length}</p></div>
                </div>
            </div>
            ...
            <div className={styles.tableCard}>
                <div className={styles.tableCardHeader}>
                    <span className={styles.tableCardTitle}>Inventaire ({filtered.length})</span>
                </div>
                <table className={styles.table}>
                    <thead className={styles.thead}>
                        <tr><th>Ressource</th><th>Catégorie</th><th style={{ textAlign: 'center' }}>Quantité</th><th>Événement assigné</th><th style={{ textAlign: 'center' }}>Disponibilité</th><th></th></tr>
                    </thead>
                            ...
                            return (
                                <tr key={r.id}>
                                    ...
                                    <td style={{ textAlign: 'right' }}>
                                        <button className={styles.actionBtnGhost} style={{ padding: '4px 10px', fontSize: '11px' }}>Détails</button>
                                    </td>
                                </tr>
                            );
                        })}
