...
const eventImages: Record<string, string> = {
    "1": "/images/event-wedding.png",
    "2": "/images/event-conference.png",
    "3": "/images/hero-events.png",
};
...
type FormData = {
    title: string; date: string; location: string; budget: string; description: string; category: string;
};
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "var(--space-lg)", padding: "var(--space-lg)" }}>
                    {filtered.map(event => (
                        <Card key={event.id} shadow="sm" style={{ overflow: "hidden" }}>
                            <div style={{ height: "160px", overflow: "hidden", position: "relative", background: "var(--color-surface-2)" }}>
                                <img src={eventImages[event.id] || "/images/hero-events.png"} alt={event.title} style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.4s" }} onMouseOver={e => (e.currentTarget.style.transform = "scale(1.05)")} onMouseOut={e => (e.currentTarget.style.transform = "scale(1)")} />
                                ...
                                <div style={{ position: "absolute", top: "var(--space-sm)", left: "var(--space-sm)", background: "rgba(0,0,0,0.5)", borderRadius: "999px", padding: "3px 10px", fontSize: "11px", color: "white", fontWeight: "700" }}>{event.category}</div>
                            </div>
                            <Card.Body>
                                <h3 style={{ fontWeight: "var(--fw-bold)", color: "var(--color-heading)", fontSize: "var(--fs-md)", margin: "0 0 var(--space-xs)" }}>{event.title}</h3>
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "var(--space-sm)", borderTop: "1px solid var(--color-border)" }}>
                                    <div style={{ display: "flex", gap: "var(--space-xs)" }}>
                                        ...
                                        <button onClick={() => openEdit(event)} className={styles.actionBtnGhost} style={{ padding: "6px 12px", fontSize: "11px", display: "flex", alignItems: "center", gap: "4px" }}><Edit size={12} /></button>
                                        <button onClick={() => handleDelete(event.id)} style={{ background: "rgba(239,83,80,0.1)", border: "none", borderRadius: "var(--radius-sm)", padding: "6px 8px", cursor: "pointer", color: "var(--color-danger)" }}><Trash2 size={12} /></button>
                                    </div>
                    ...
                    <Card style={{ width: "560px", maxWidth: "95vw", maxHeight: "90vh", overflowY: "auto" }}>
                        ...
                        <Card.Body>
                            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "var(--space-md)" }}>
                                {[{ label: "Titre de l'événement", key: "title", placeholder: "Ex: Mariage, Conférence...", type: "text" },
                                { label: "Date", key: "date", placeholder: "", type: "date" },
                                { label: "Lieu", key: "location", placeholder: "Ville ou salle...", type: "text" },
                                { label: "Budget (€)", key: "budget", placeholder: "0", type: "number" }].map(f => (
                                    <div key={f.key}>
                                        <label style={{ fontSize: "var(--fs-xs)", fontWeight: "var(--fw-bold)", color: "var(--color-muted)", display: "block", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.04em" }}>{f.label}</label>
                                ...
                                <div>
                                    <label style={{ fontSize: "var(--fs-xs)", fontWeight: "var(--fw-bold)", color: "var(--color-muted)", display: "block", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.04em" }}>Description</label>
                                    <textarea rows={3} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Détails de l'événement..." style={{ width: "100%", padding: "10px 14px", borderRadius: "var(--radius-md)", border: "1px solid var(--color-border)", background: "var(--color-surface-2)", color: "var(--color-text)", fontSize: "var(--fs-sm)", outline: "none", resize: "vertical", fontFamily: "var(--font-family-base)" }} />
                                </div>
                ...
                <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "var(--space-md)" }}>
                    <Card style={{ width: "600px", maxWidth: "95vw" }}>
                        <div style={{ height: "220px", overflow: "hidden" }}>
                            <img src={eventImages[showDetailModal.id] || "/images/hero-events.png"} alt={showDetailModal.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                        </div>
                        <Card.Body>
