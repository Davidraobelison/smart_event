    ...
    return (
        <div className={taskStyles.calendarBox}>
            <div className={taskStyles.calHeader}>
                <button className={taskStyles.calNav} onClick={prev}><ArrowLeft size={16} /></button>
                <span className={taskStyles.calMonthTitle}>{MONTHS[month]} {year}</span>
                <button className={taskStyles.calNav} onClick={next}><ArrowRight size={16} /></button>
            </div>
            <div className={taskStyles.calDays}>
                {DAYS.map(d => <div key={d} className={taskStyles.calDayLabel}>{d}</div>)}
            </div>
...
export default function TimelineTachePage() {
    ...
    const openCreate = (status: Status = "TODO") => {
        setEditId(null); setForm({ ...emptyForm, status }); setShowModal(true);
    };
    const openEdit = (task: Task) => {
        setEditId(task.id); setForm({ title: task.title, desc: task.desc, priority: task.priority, deadline: task.deadline, assignee: task.assignee, status: task.status, eventName: task.eventName }); setShowModal(true);
    };
                    ...
                    <Card style={{ width: "500px", maxWidth: "95vw" }}>
                        ...
                        <Card.Body>
                            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "var(--space-md)" }}>
                                {[{ label: "Titre", key: "title", type: "text", placeholder: "Ex: Confirmer la salle..." },
                                { label: "Description", key: "desc", type: "text", placeholder: "Détails de la tâche..." },
                                { label: "Responsable", key: "assignee", type: "text", placeholder: "Ex: JD" },
                                { label: "Échéance", key: "deadline", type: "date", placeholder: "" }].map(f => (
                                    <div key={f.key}>
