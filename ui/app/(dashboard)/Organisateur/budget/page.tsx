...
const initialItems = [
    ...
    { id: "3", category: "Animation", item: "DJ & Sistem Sono", planned: 3000, actual: 3000, status: "paid" },
        ...
        <div className={styles.page}>
            ...
            <div className={styles.statGrid}>
                <div className={styles.statCard}>
                    ...
                    <div>
                        <p className={styles.statLabel}>Budget Prévu</p>
                        <p className={styles.statValue}>{totalPlanned.toLocaleString('fr-FR')} €</p>
                    </div>
                </div>
                <div className={styles.statCard}>
                    <div className={styles.statIconBox} style={{ background: 'rgba(36,180,126,0.1)', color: 'var(--color-success)' }}><TrendingUp size={28} /></div>
                    <div>
                        <p className={styles.statLabel}>Réel Décaissé</p>
                        <p className={styles.statValue}>{totalActual.toLocaleString('fr-FR')} €</p>
                    </div>
                </div>
                <div className={styles.statCard}>
                    <div className={styles.statIconBox} style={{ background: 'rgba(255,176,32,0.1)', color: 'var(--color-warning)' }}><AlertCircle size={28} /></div>
                    <div>
                        <p className={styles.statLabel}>Solde Restant</p>
                        <p className={styles.statValue} style={{ color: remaining < 0 ? 'var(--color-danger)' : 'inherit' }}>{remaining.toLocaleString('fr-FR')} €</p>
                    </div>
                </div>
            </div>
